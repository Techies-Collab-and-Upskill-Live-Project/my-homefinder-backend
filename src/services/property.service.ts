...
export class PropertyService {
    ...
    public uploadPropertyImages = async (images: MulterFile[], propertyId: string) => {
    ...
    public getPropertiesInLocation = async (location: string, radius: number, page: number, limit: number) => {
        ...
        const countResult: any = await this.prisma.$queryRaw`SELECT COUNT(*)
                                                             FROM (SELECT *,
                                                                          (6371 * acos(
